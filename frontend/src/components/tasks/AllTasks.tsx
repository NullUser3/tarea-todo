...
import { TaskResponseType } from "../customHooks/usePersonalTask";

type AllTasksProps = {
  tasks: TaskResponseType[] | null; // Replace TaskType with your actual task type
};

export const AllTasks = ({ tasks }: AllTasksProps) => {
  const { value, setCurrentTask, currentTask } = usePersonalTaskContext();
  const { handleTaskDelete, editTask } = value;
